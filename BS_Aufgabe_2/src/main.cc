...
// der Wert sollte variiert werden oder man bastelt sich lieber eine rein
// "zeitabhängige" Pause ;)
#define PAUSE 100000
...
int main() {
    ...
    for (int blink = 0; blink < 2; blink++) {
        for (int bg = 0; bg < 8; bg++) {
            for (int fg = 0; fg < 16; fg++) {
                cga.setAttr(CgaAttr((CgaAttr::Color)(fg), (CgaAttr::Color)(bg),
                                    (CgaAttr::Color)blink));
                out.print("X");
            }
        }
    ...
    cga.clear();
    cga.setAttr(CgaAttr((CgaAttr::Color)CgaAttr::WHITE,
                        (CgaAttr::Color)CgaAttr::BLACK,
                        (CgaAttr::Color) false));
    out.println();
    out.println(
        "Hier Bildschirm sollten im unteren Bereich bunte Spalten erscheinen. "
        "Bei diesen sollten jeweils im Wechsel acht Spalten ohne und acht "
        "Spalten mit dem #-Zeichen versehen sein. Darueber hinaus sollten "
        "diese kaskadierend blinken. Das Bild muss mit einer schwarzen Spalte "
        "beginnen.");
    out.println();
    ...
    out.println(
        "Hier stehen jetzt fuenf Worte\nund auf der naechsten Zeile geht es "
    ...
    out.println(
        "Das ist ein Testsatz, der laenger als 80 Zeichen sein soll. Mal "
        "sehen, ob der Zeilenumbruch klappt. Haha Haha Haha Haha Haha Haha "
        "Haha Haha.\n");

    out.println(
        "Gleich wird das Scrollen getestet. Das Scrollen sollte uebrigens mit "
        "der selber zu schreibenden memcpy-Methode geloest werden - "
        "Zusatzaufgabe (und nicht mit einer einfachen Zuweisung).");
    out.println("\n");
    // kurze Pause
    ...
    for (int i = 0; i < 40; i++) {
        cga.setAttr(CgaAttr((CgaAttr::Color)(i % 16), (CgaAttr::Color)(8),
                            (CgaAttr::Color) false));
        out.print(
            "Gleich wird das Scrollen getestet. Das Scrollen sollte uebrigens "
            "mit der selber zu schreibenden memcpy-Methode geloest werden - "
            "Zusatzaufgabe (und nicht mit einer einfachen Zuweisung).");
    };
